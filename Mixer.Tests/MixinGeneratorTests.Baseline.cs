// Copyright 2023 Subatomix Research Inc.
// SPDX-License-Identifier: ISC

namespace Mixer;

partial class MixinGeneratorTests
{
    [Test]
    public void Baseline_Nongeneric()
    {
        var result = RunMixinGenerator(
            "using Mixer; namespace Test { [Mixin]                   class Source { } }",
            "using Mixer; namespace Test { [Include(typeof(Source))] class Target { } }"
        );

        result.GeneratedSources.Should().BeEquivalentTo(new[]
        {
            Source("Test.Target.0.g.cs",
                """
                // <auto-generated>
                //   This code file was generated by the 'Mixer' NuGet package.
                //   See https://github.com/sharpjs/Mixer for more information.
                // </auto-generated>
                #nullable enable
                namespace Test;
                partial class Target
                {
                }
                """
            )
        });

        result.Diagnostics.Should().BeEmpty();
    }

    [Test]
    public void Baseline_Generic()
    {
        var result = RunMixinGenerator(
            "using Mixer; namespace Test { [Mixin]           class Source { } }",
            "using Mixer; namespace Test { [Include<Source>] class Target { } }"
        );

        result.GeneratedSources.Should().BeEquivalentTo(new[]
        {
            Source("Test.Target.1.g.cs",
                """
                // <auto-generated>
                //   This code file was generated by the 'Mixer' NuGet package.
                //   See https://github.com/sharpjs/Mixer for more information.
                // </auto-generated>
                #nullable enable
                namespace Test;
                partial class Target
                {
                }
                """
            )
        });

        result.Diagnostics.Should().BeEmpty();
    }
}
